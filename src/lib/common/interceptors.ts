...
import { NetworkConnecting, Refreshing } from '$lib/store/apistore';
...
const refresh_url = import.meta.env.VITE_API_ENDPOINT + 'member/refresh_token';

//토큰 재발행 요청
export const setInterceptors = (instance: any) => {
	let isTokenRefreshing: boolean;
	...
	instance.interceptors.request.use(
		(config: any) => {
			if (dev) {
				...
				if (config.method === 'post') console.log('Post DParams', config.data);
				else console.log('Get DParams', config.params);
			}

			// request 보내기전 작업 헤더에 인증 토큰 추가
			const accessToken: string | null | boolean = browser && localStorage.getItem('accessToken');
