<script lang="ts">
	...
	const reqCode = async () => {
		...
		const { data } = await instance.post('addon/send_msg/Req_code', params);
		isReqCode = false;
	...
	const sendCode = async () => {
		...
		const { data } = await instance.post('addon/send_msg/Send_code', params);
		getAuth = data?.data?.get_auth;
		if (dev) console.log('D ', getAuth);
		if (!isRegister && getAuth) goto('./');
	};
	let btnTxt = '로그인';
...
<div class="wrap-box">
	...
	{#if authCode}
		<div class="mt-4" in:scale={{ duration: 150 }}>
			...
			{:else}
				<div class="mt-4" in:scale={{ duration: 150 }}>
					{#if isRegister}
						<span class="text-tertiary-500">회원가입된 이메일입니다. </span>
					{:else}
	...
	{/if}
	{#if getAuth}
		<div class="mt-4" in:scale={{ duration: 150 }}>회원가입되었습니다. 닉네임을 설정하세요.</div>
	{/if}
</div>
